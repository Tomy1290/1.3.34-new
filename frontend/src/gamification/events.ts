...
export const EVENTS: WeeklyEvent[] = [
  { id: 'water_boost', title:(l)=> l==='de'? 'Hydration Boost' : 'Hydration Boost', description:(l)=> l==='de'? 'Erreiche an 4 Tagen dein Wasserziel.' : 'Hit your personal water goal on 4 days.', progress:(keys,s)=> pct(countDays(keys,s,(d)=> { const wkg = computeDailyWaterTargetMl(undefined, !!d?.drinks?.sport); const intakeMl = ((s as any).waterCupMl || 250) * (d?.drinks?.water ?? 0) + (d?.drinks?.waterCure ? 1000 : 0); return intakeMl >= Math.max(1,wkg); }), 4), xp: 120, bonusPercent: 0.10 },
