...
export const EVENTS: WeeklyEvent[] = [
  ...
  { id: 'water_plus', title:(l)=> l==='de'? 'Mehr Wasser' : 'More Water', description:(l)=> l==='de'? 'Wasserziel an 5 Tagen.' : 'Hit personal water goal 5 days.', progress:(k,s)=> pct(countDays(k,s,(d)=> { const wkg = computeDailyWaterTargetMl(undefined, !!d?.drinks?.sport); const intakeMl = ((s as any).waterCupMl || 250) * (d?.drinks?.water ?? 0) + (d?.drinks?.waterCure ? 1000 : 0); return intakeMl >= Math.max(1,wkg); }), 5), xp: 140, bonusPercent: 0.10 },
