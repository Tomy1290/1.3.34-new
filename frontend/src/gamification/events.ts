...
export const EVENTS: WeeklyEvent[] = [
  ...
  { id: 'water_streak', title:(l)=> l==='de'? 'Wasser-Streak' : 'Water Streak', description:(l)=> l==='de'? '2 Tage in Folge Wasserziel.' : 'Water goal 2 days in a row.', progress:(keys,s)=> { let best=0,cur=0; for(const k of keys){ const d = (s as any).days[k]; const wkg = computeDailyWaterTargetMl(undefined, !!d?.drinks?.sport); const intakeMl = ((s as any).waterCupMl || 250) * (d?.drinks?.water ?? 0) + (d?.drinks?.waterCure ? 1000 : 0); const ok = intakeMl >= Math.max(1,wkg); if(ok){ cur+=1; best=Math.max(best,cur);} else cur=0; } return pct(best,2); }, xp: 120, bonusPercent: 0.10 },
