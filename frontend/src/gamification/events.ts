...
export const EVENTS: WeeklyEvent[] = [
  { id: 'water_boost', title:(l)=> l==='de'? 'Hydration Boost' : 'Hydration Boost', description:(l)=> l==='de'? 'Erreiche an 4 Tagen dein Wasserziel.' : 'Hit your personal water goal on 4 days.', progress:(keys,s)=> pct(countDays(keys,s,(d)=> { const wkg = getLatestWeightKg((s as any).days); const goalMl = computeDailyWaterTargetMl(wkg, !!d?.drinks?.sport); const intakeMl = ((s as any).waterCupMl || 250) * (d?.drinks?.water ?? 0) + (d?.drinks?.waterCure ? 1000 : 0); return intakeMl >= Math.max(1,goalMl); }), 4), xp: 120, bonusPercent: 0.10 },
  ...
  { id: 'perfect_day_duo', title:(l)=> l==='de'? 'Perfekt x2' : 'Perfect x2', description:(l)=> l==='de'? 'Schaffe 2 perfekte Tage.' : 'Achieve 2 perfect days.', progress:(keys,s)=> pct(countDays(keys,s,(d)=> { if(!d) return false; const pills=!!d?.pills?.morning && !!d?.pills?.evening; const wkg = getLatestWeightKg((s as any).days); const goalMl = computeDailyWaterTargetMl(wkg, !!d?.drinks?.sport); const intakeMl = ((s as any).waterCupMl || 250) * (d?.drinks?.water ?? 0) + (d?.drinks?.waterCure ? 1000 : 0); const water = intakeMl >= Math.max(1,goalMl); const weight=typeof d?.weight==='number'; return pills && water && weight; }), 2), xp: 150, bonusPercent: 0.15 },
  ...
  { id: 'water_streak', title:(l)=> l==='de'? 'Wasser-Streak' : 'Water Streak', description:(l)=> l==='de'? '2 Tage in Folge Wasserziel.' : 'Water goal 2 days in a row.', progress:(keys,s)=> { let best=0,cur=0; const wkg = getLatestWeightKg((s as any).days); for(const k of keys){ const d = (s as any).days[k]; const goalMl = computeDailyWaterTargetMl(wkg, !!d?.drinks?.sport); const intakeMl = ((s as any).waterCupMl || 250) * (d?.drinks?.water ?? 0) + (d?.drinks?.waterCure ? 1000 : 0); const ok = intakeMl >= Math.max(1,goalMl); if(ok){ cur+=1; best=Math.max(best,cur);} else cur=0; } return pct(best,2); }, xp: 120, bonusPercent: 0.10 },
  ...
  { id: 'chat_week', title:(l)=> l==='de'? 'Chat-Woche' : 'Chat Week', description:(l)=> l==='de'? '5 Nachrichten mit Gugi.' : 'Send 5 messages to Gugi.', progress:(keys,s)=> { const { start, end } = getWeekRange(new Date(keys[0])); const msgs = ((s as any).chat || []) as { sender: 'user'|'bot'; createdAt: number }[]; const count = msgs.filter(m => m.sender==='user' && typeof m.createdAt==='number' && m.createdAt >= +start && m.createdAt <= +end).length; return Math.max(0, Math.min(100, Math.floor((count/5)*100))); }, xp: 90, bonusPercent: 0.05 },
  { id: 'water_plus', title:(l)=> l==='de'? 'Mehr Wasser' : 'More Water', description:(l)=> l==='de'? 'Wasserziel an 5 Tagen.' : 'Hit personal water goal 5 days.', progress:(k,s)=> pct(countDays(k,s,(d)=> { const wkg = getLatestWeightKg((s as any).days); const goalMl = computeDailyWaterTargetMl(wkg, !!d?.drinks?.sport); const intakeMl = ((s as any).waterCupMl || 250) * (d?.drinks?.water ?? 0) + (d?.drinks?.waterCure ? 1000 : 0); return intakeMl >= Math.max(1,goalMl); }), 5), xp: 140, bonusPercent: 0.10 },
  ...
  { id: 'perfect_triple', title:(l)=> l==='de'? 'Perfekt x3' : 'Perfect x3', description:(l)=> l==='de'? '3 perfekte Tage.' : '3 perfect days.', progress:(k,s)=> pct(countDays(k,s,(d)=> { if(!d) return false; const p=!!d?.pills?.morning && !!d?.pills?.evening; const wkg = getLatestWeightKg((s as any).days); const goalMl = computeDailyWaterTargetMl(wkg, !!d?.drinks?.sport); const intakeMl = ((s as any).waterCupMl || 250) * (d?.drinks?.water ?? 0) + (d?.drinks?.waterCure ? 1000 : 0); const w = intakeMl >= Math.max(1,goalMl); const g=typeof d?.weight==='number'; return p && w && g; }), 3), xp: 180, bonusPercent: 0.15 },
...
export function computeEventProgress(dayKeys: string[], state: Pick<AppState, 'days'>, evt: WeeklyEvent) { const percent = Math.max(0, Math.min(100, Math.floor(evt.progress(dayKeys, state)))); return { percent, completed: percent >= 100 }; }