...
export const EVENTS: WeeklyEvent[] = [
  ...
  { id: 'perfect_day_duo', title:(l)=> l==='de'? 'Perfekt x2' : 'Perfect x2', description:(l)=> l==='de'? 'Schaffe 2 perfekte Tage.' : 'Achieve 2 perfect days.', progress:(keys,s)=> pct(countDays(keys,s,(d)=> { if(!d) return false; const pills=!!d?.pills?.morning && !!d?.pills?.evening; const wkg = computeDailyWaterTargetMl(undefined, !!d?.drinks?.sport); const intakeMl = ((s as any).waterCupMl || 250) * (d?.drinks?.water ?? 0) + (d?.drinks?.waterCure ? 1000 : 0); const water = intakeMl >= Math.max(1,wkg); const weight=typeof d?.weight==='number'; return pills && water && weight; }), 2), xp: 150, bonusPercent: 0.15 },
